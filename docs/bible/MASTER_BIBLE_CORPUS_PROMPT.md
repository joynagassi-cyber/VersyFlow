...
| D5 | Comptable de référence par dataset | Chaque dataset a son propre `expectedVerseCount` mesuré sur la source USFM réelle (eBible.org), pas un comptable canonique global. Le LSG eBible fait ~31 170 versets, Ostervald ~31 107. Le comptable « 17 380 » de l'ancien corpus placeholder n'est PAS un invariant. Le validator compare au comptable du dataset (§53 : mismatch = warning, pas erreur bloquante). |
